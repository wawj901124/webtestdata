...
class TestSinglePageMeminfoClass(unittest.TestCase):  # 创建测试类
...
def __generateTestCases():
    ...
    meminfotestcase_all = MeminfoTestCase.objects.filter(testproject="Qrindo").order_by('id')
    ...
    for meminfotestcase in meminfotestcase_all:
        ...
        for i in range(1,forcount+1):  # 循环，从1开始
            ...
            setattr(TestSinglePageMeminfoClass, 'test_func_%s_%s_%s' % (meminfotestcaseid,meminfotestcase.testcasetitle,forcount_i),
                    TestSinglePageMeminfoClass.getTestFunc(*args))  # 通过setattr自动为TestCase类添加成员方法，方法以“test_func_”开头
