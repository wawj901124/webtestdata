...
class TestTopUpBalanceClass(unittest.TestCase):  # 创建测试类
    ...
    def definedepend(self,dependid):
        print("依赖ID（dependid）为:%s" % dependid)
        if dependid != None:
            ...
            self.baseframe.outPutMyLog("依赖ID为None，不执行依赖！")
...
def __generateTestCases():
    ...
    meminfotestcase_all = MeminfoTestCase.objects.filter(testproject="Qrindo").filter(testpage="TopUpBalance页").order_by('id')
    ...
    for meminfotestcase in meminfotestcase_all:
        ...
        for i in range(1,forcount+1):  # 循环，从1开始
            ...
            setattr(TestTopUpBalanceClass, 'test_func_%s_%s_%s' % (meminfotestcaseid,meminfotestcase.testcasetitle,forcount_i),
                    TestTopUpBalanceClass.getTestFunc(*args))  # 通过setattr自动为TestCase类添加成员方法，方法以“test_func_”开头
