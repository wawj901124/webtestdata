...
class TestCouponsIssuanceListSearchClass(unittest.TestCase):  # 创建测试类
    ...
    #定义搜索查找函数
    def definesearch(self,num,isclicklastpage=None,selectxpath=None,selectoptiontextxpath=None,selectinputxpath=None,selectinputselectonexpath=None,selectinputtext=None,isfind=False,colnum=None,checktext=None):
        ...
        #点击最后一页
        if isclicklastpage:
            self.activeweb.findElementByXpathAndScriptClick(self.testpage.last_page)
...
def __generateTestCases():
    ...
    searchdata_all = SearchData.objects.filter(webproject=u"营销系统").filter(testpage=u"优惠券发放查询列表").filter(isclicklastpage=True).order_by(
        'id')
    ...
    for searchdata in searchdata_all:
        ...
        args.append(searchdata.isclicklastpage)
