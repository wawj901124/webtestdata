...
class TestEditActivityClass(unittest.TestCase):  # 创建测试类
    ...
    def defineeditactivity(self,num,
        ...
        if ISXIANGWANG:
            if not self.activeweb.getEleImage(num, self.ticketcreatepage.sypt_mbmpay_checkbox).is_selected():
                self.activeweb.findElementByXpathAndClickNum(num,self.ticketcreatepage.sypt_mbmpay_checkbox)  # 使用平台点选mbmpay
            if self.activeweb.getEleImage(num, self.ticketcreatepage.sypt_mydisrupto_checkbox).is_selected():
                self.activeweb.findElementByXpathAndClickNum(num,self.ticketcreatepage.sypt_mydisrupto_checkbox)  # 使用平台点选mydisrupto
            if self.activeweb.getEleImage(num, self.ticketcreatepage.sypt_QRindo_checkbox).is_selected():
...
def __generateTestCases():
    ...
    addactivity_all = AddActivity.objects.filter(testproject="营销系统").filter(testmodule="任务活动管理").filter(testpage="编辑活动").filter(id=18).order_by('id')
