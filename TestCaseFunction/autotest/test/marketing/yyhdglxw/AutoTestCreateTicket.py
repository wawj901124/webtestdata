...
class TestCreateTicketClass(unittest.TestCase):  # 创建测试类
    ...
    def definecreateticket(self,num,ffzt,kcslinputtext,qyxq,
        ...
        self.activeweb.findElementByXpathAndClickNum(num, self.activitycreatepage.jy_wcjy_jylx_checkbox)  # 点击交易类型全选框
...
def __generateTestCases():
    ...
    addticket_all = AddTicket.objects.filter(testproject="营销系统").filter(testmodule="任务活动管理").filter(testpage="创建代金券").filter(id=5).order_by('id')
