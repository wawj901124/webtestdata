...
from webtestdata.settings import WEB_URL_TITLE,MANAGER_LOGIN_ACCOUNT,MANAGER_LOGIN_PASSWORD,MARKETING_HIGH_LINES_ACTIVITYID
...
# from TestCaseFunction.autotest.config.page.marketing.yyhdglxw.activityCreatePage import ActivityCreatePage   #导入创建活动页
from TestCaseFunction.autotest.config.page.marketing.yyhdglxw.activityEditPage import ActivityEditPage  #导入未上线活动编辑页
from TestCaseFunction.autotest.config.page.marketing.yyhdglxw.ticketCreatePage import TicketCreatePage   #导入创建优惠券页
from TestCaseFunction.autotest.config.page.marketing.yyhdglxw.activityListPage import ActivityListPage   #导入活动列表页


class TestHighLinesActivityClass(unittest.TestCase):  # 创建测试类
    ...
    # 任务类型为1表示注册，为2表示交易类型
    #奖励类型1表示固定奖励
    def definehighlinesactivity(self,num,sxhdmcinputtext,hdmcinputtext):
        ...
        self.activeweb.findElementByXpathAndScriptClickNum(num, self.testpage.table_justone_content_sx)  # 点击上线按钮
        self.activeweb.findElementByXpathAndScriptClickNum(num, self.testpage.first_sx_popup_qd)  # 点击上线弹框中确定
        #断言待上线活动变为进行中
    ...
    def getTestFunc(num,sxhdmcinputtext,hdmcinputtext):
        def func(self):
            self.definehighlinesactivity(num,sxhdmcinputtext,hdmcinputtext)
        return func
