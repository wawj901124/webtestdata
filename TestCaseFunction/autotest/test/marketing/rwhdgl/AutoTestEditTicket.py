...
from TestCaseFunction.autotest.config.page.marketing.rwhdgl.ticketEditPage import TicketEditPage #导入未上线活动优惠券编辑页
...
class TestEditTicketClass(unittest.TestCase):  # 创建测试类
    ...
    def definecreateticket(self,num,ffzt,kcslinputtext,qyxq,
        ...
        if ISONLINE:
            # 点击投放渠道二级渠道第一个选项
            if not self.activeweb.findElementByXpath(self.activityeditpage.tfqd_select_nbqd_fxk_mbmpay_checkbox).is_selected():
            ...
            if self.activeweb.findElementByXpath(
                    self.activityeditpage.tfqd_select_nbqd_fxk_mydisrupto_checkbox).is_selected():
                self.activeweb.findElementByXpathAndClickNum(num, self.activityeditpage.tfqd_select_nbqd_fxk_mydisrupto_checkbox)

            if self.activeweb.findElementByXpath(
                    self.activityeditpage.tfqd_select_nbqd_fxk_qrindo_checkbox).is_selected():
                self.activeweb.findElementByXpathAndClickNum(num, self.activityeditpage.tfqd_select_nbqd_fxk_qrindo_checkbox)
        else:
            # 点选投放渠道二级渠道第一项
            self.activeweb.findElementByXpathAndClickNum(num, self.activityeditpage.tfqd_select_nbqd_fxk_mbmpay_checkbox)
