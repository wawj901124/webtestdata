...
class ReportAdmin(object):
    ...
    list_display =['id','reportname', 'go_to','add_time','update_time']#定义显示的字段
    search_fields =  ['reportname',]   #定义搜索字段
    list_filter =  ['reportname',] #定义筛选的字段
    model_icon = 'fa fa-tasks'  # 定义图标显示
    ordering = ['-add_time']  # 添加默认排序规则显示排序，根据添加时间倒序排序
    readonly_fields = ['id','reportname', 'reportfile','add_time','update_time',]  # 设置某些字段为只为可读  #设置了readonly_fields，再设置exclude，exclude对该字段无效，
    ...
    # list_display_links = ['reportname',]   #设置点击链接进入编辑页面的字段
    # date_hierarchy = 'add_time'   #详细时间分层筛选，未生效
    # show_detail_fields = ['reportname',]   #显示数据详情
